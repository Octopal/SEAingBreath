...
namespace GVR.Visual {
	...
	public class ScreenFader : MonoBehaviour {
		...
		private ScreenFader[] subordinates = new ScreenFader[0];
		...
		void Start() {
			...
			subordinates = new ScreenFader[cameras.Length - 1];
			int filled = 0;
			for (int i = 0; i < cameras.Length; i++) {
				if (cameras[i].gameObject != gameObject) {
					subordinates[filled] = cameras[i].gameObject.AddComponent<ScreenFader>();
